...
Learned that `useState` triggers a re-render of the component, Which means is that, the Component function is called, and run again. Now I get the answer to how the `setName` can magically change the value of `name` when it is a already destructured.
