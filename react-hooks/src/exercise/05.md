...
I learned that we need to destroy danggling listeners in useEffect cleanup callback. and useRef is used on the DOM to get it's DOM value. With this, I also, understand something interesting that, if none of what I write is actual DOM in render function, all the callback functions events that we listen to on elements are proxied by react as well. they are not real events ?
